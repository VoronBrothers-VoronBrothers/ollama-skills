...
if [ -z "$WIN" ]; then
  ...
  # Поллинг окна с гранулярностью 0.2с, дедлайн 25с
  DEADLINE=$((SECONDS + 25))
...
xdotool windowmove "$WIN" 0 0 windowsize "$WIN" 1920 1024
sleep 1

# New chat → поле ввода (Chrome обязан быть активным окном)
if [ "$(xdotool getactivewindow 2>/dev/null)" != "$WIN" ]; then
  xdotool windowactivate --sync "$WIN" 2>/dev/null || xdotool windowfocus "$WIN"
fi
[ "$(xdotool getactivewindow 2>/dev/null)" = "$WIN" ] || { echo "FAIL: Chrome не активное окно (активно: $(xdotool getactivewindow 2>/dev/null))"; exit 5; }
xdotool mousemove 93 213 click 1; sleep 1.0
xdotool mousemove 960 570 click 1; sleep 0.5
